...
<NoteFigure src="/marginalia/vpn-namespace.svg" alt="The torrent client shares the VPN container's network namespace, so all its traffic exits through the VPN tunnel and it has no interface of its own.">The torrent client borrows the VPN's interface. The kill-switch is the wiring, not a setting.</NoteFigure>
