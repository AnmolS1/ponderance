...
<figure>
  <img src="/marginalia/embedding-space.svg" alt="A text query and a photo are each encoded into the same vector space; search returns the photo whose vector has the smallest angle (cosine distance) to the query." />
  <figcaption>Words and photos land in one space; search is just the smallest angle.</figcaption>
</figure>
