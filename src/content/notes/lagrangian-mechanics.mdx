...
The principle of least action says: the physical path is the one where $\delta S = 0$ — small variations around the true path produce no first-order change in $S$.
...
Since this must hold for all $\eta$, the integrand itself must vanish — the **Euler-Lagrange equation**:
...
Both converge to the same trajectory for small angles. For $\theta_0 \lesssim 20°$, $\sin\theta \approx \theta$ and the period is $T \approx 2\pi\sqrt{\ell/g}$ — independent of amplitude, which is the famous isochronous result Galileo observed.

## Why this matters for ML

The path-integral formulation of quantum mechanics is a direct generalization: the probability amplitude for a particle to travel from $A$ to $B$ is a sum over *all* paths, weighted by $e^{iS/\hbar}$. The Lagrangian framing unifies classical mechanics, field theory, and quantum theory in a single notation. Once you see the world through $\delta S = 0$, the equations of motion for everything — electrodynamics, general relativity, the Standard Model — follow from the same one-line principle.
